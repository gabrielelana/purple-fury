...
* Server Side [2/13]
  ...
  - [ ] Add ~nodemon~ to restart the server on change
  - [ ] Add middleware to serve the favicon
  - [ ] Login an user anonymously
  - [ ] Login an user with username/password and lock that username
  - [ ] ???
